...
macOS menu bar app: select text or capture a screen region, press a hotkey, ask AI, and read the answer in a panel anchored next to the content. Distributed as a notarized DMG on GitHub, not the App Store. Not sandboxed.
...
- `Sources/PeekCapture` - `SelectionReader` and `ScreenRegionCapturer` implementations (Accessibility API, ScreenCaptureKit).
